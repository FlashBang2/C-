...
void main() {
    ...
    if(horizontal) {
        for(int i = 1; i < radius; i++) {
            ...
            result += texture(screenTexture, texCoords - vec2(tex_offset.x * i, 0.0)).rgb * weights[i];
            result += texture(screenTexture, texCoords + vec2(tex_offset.x * i, tex_offset.y * i)).rgb * weights[i];
            result += texture(screenTexture, texCoords - vec2(tex_offset.x * i, tex_offset.y * i)).rgb * weights[i];
            result += texture(screenTexture, texCoords + vec2(0.0, tex_offset.y * i)).rgb * weights[i];
            result += texture(screenTexture, texCoords - vec2(0.0, tex_offset.y * i)).rgb * weights[i];
            result += texture(screenTexture, texCoords + vec2(0.0, 0.0)).rgb * weights[i];
            result += texture(screenTexture, texCoords - vec2(0.0, 0.0)).rgb * weights[i];
        }
    }
